/*
 * datawrapper / vis / stacked-column-chart v1.5.1
 * generated on 2016-04-06T11:32:34+02:00
 */
/*! datawrapper - v1.10.2 */
...
me.init();var lh=0,lhi=0,lw=0,lwi=0;barColumns=me.getBarColumns(),n=barColumns.length;_.each(barColumns,function(column,s){var lbl_w=me.labelWidth(column.title(),'series'),lbl_h=me.labelHeight(column.title(),'series');if(lbl_w>=lw){lw=lbl_w;lwi=s;}
if(lbl_h>=lh){lh=lbl_h;lhi=s;}});var bw=(c.w*0.9)/(me.axesDef.columns.length*1.5);if(bw<20){c.bpad+=me.labelWidth(barColumns[lwi].title(),'series smaller');}else if(bw<30){c.bpad+=lw;}else{c.bpad+=lh;}
me.initDimensions();me.__bars={};me.__barLbls={};me.__gridlines={};me.__gridlabels={};me.__series_names={};if(!me.theme().columnChart.cutGridLines)me.horzGrid();me.update();el.mousemove(_.bind(me.onMouseMove,me));if(dataset.numRows()>1){var items=[],lblFmt=me.chart().columnFormatter(me.axes(true).labels);dataset.eachRow(function(r){items.push({label:lblFmt(me.axes(true).labels.val(r)),color:me.getColor(null,r,{varyLightness:true,key:me.axes(true).labels.val(r)})});});me.addLegend(items,$('#header',c.root.parent()));}
